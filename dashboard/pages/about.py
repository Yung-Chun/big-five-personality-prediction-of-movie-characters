...
data = pd.read_csv('./movie_char_big_five_predict_result.csv')

all_records = pd.DataFrame()

result_path = Path('./training_result')
for file in result_path.glob(f'*svc_essays_embedding*'):

    temp = pd.read_csv(result_path / f'{file.stem}.csv')
...
for n in range(0, len(y_cols)):

    fig.add_trace(
        go.Scatter(
            x=all_records[all_records.y == y_cols[n]]['C'],
            y=all_records[all_records.y == y_cols[n]]['mean_train_score'],
            name=f'Mean Train Score of {y_cols[n]}',
            mode='lines+markers',
    ...
    fig.add_trace(
        go.Scatter(
            x=all_records[all_records.y == y_cols[n]]['C'],
            y=all_records[all_records.y == y_cols[n]]['mean_test_score'],
            name=f'Mean Test Score of {y_cols[n]}',
            mode='lines+markers',
